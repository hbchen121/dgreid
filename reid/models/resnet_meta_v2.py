...
from reid.layers.smm_layer import MetaStyleMixtureModule as SMMLayer
...
class ResNet(nn.Module):
    ...
    def backbone_forward(self, x, meta_train=False, meta_test_domain=None):
        if self.with_smm and -1 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[0](x, meta_train)
            else:
                x = self.smm[0](x, meta_train)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if self.with_smm and 0 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[1](x, meta_train)
            else:
                x = self.smm[1](x, meta_train)
        ...
        if self.with_smm and 1 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[2](x, meta_train)
            else:
                x = self.smm[2](x, meta_train)
        ...
        if self.with_smm and 2 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[3](x, meta_train)
            else:
                x = self.smm[3](x, meta_train)
        ...
        if self.with_smm and 3 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[4](x, meta_train)
            else:
                x = self.smm[4](x, meta_train)
        ...
        if self.with_smm and 4 in self.smm_stage:
            if self.with_smm_loss and self.training:
                x, mix_mean, mix_std, style_mean, style_std = self.smm[5](x, meta_train)
            else:
                x = self.smm[5](x, meta_train)
        if self.with_smm and self.with_smm_loss and self.training:
            return x, mix_mean, mix_std, style_mean, style_std
        return x

    def heads_forward(self, x, meta_train=False, meta_test_domain=None):
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        ...
        if self.num_classes[0] != 0:
            if meta_train:
                bn_x = torch.chunk(bn_x, self.n_source - 1, dim=0)
                j = 0
                for i in range(self.n_source):
                    if i == meta_test_domain:
                        continue
                    if self.classifiers:
                        prob = self.classifiers[i](bn_x[j])
                        probs.append(prob)
                    if self.global_classifier:
                        global_prob = self.global_classifier(bn_x[j])
                        g_probs.append(global_prob)
                    j += 1
            else:
                if self.classifiers:
                    probs = self.classifiers[meta_test_domain](bn_x)
                if self.global_classifier:
                    g_probs = self.global_classifier(bn_x)

        if meta_train:
            x_n = torch.chunk(x, self.n_source - 1, dim=0)
        else:
            x_n = x

        if self.norm:
            if meta_train:
                norm_bn_x = torch.chunk(norm_bn_x, self.n_source - 1, dim=0)
            return probs, g_probs, x_n, norm_bn_x
        else:
            return probs, g_probs, x_n

    def forward(self, x, meta_train=False, meta_test_domain=None):
        if self.with_smm and self.with_smm_loss and self.training:
            x, mix_mean, mix_std, style_mean, style_std = self.backbone_forward(x, meta_train, meta_test_domain)
            x = self.heads_forward(x, meta_train, meta_test_domain)
            return x, mix_mean, mix_std, style_mean, style_std
        x = self.backbone_forward(x, meta_train, meta_test_domain)
        x = self.heads_forward(x, meta_train, meta_test_domain)
        return x
...
def resnet_meta_v2(depth, pretrained=True, cut_at_pooling=False,
...
def resnet50_meta_v2(**kwargs):
    return resnet_meta_v2('50x', **kwargs)
