...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, with_in,
                 ...
                 cut_at_pooling, num_features, norm, dropout, num_classes=0):
        self.inplanes = 64
        super().__init__()

        self.n_source = len(num_classes) if isinstance(num_classes, list) else 1  # 兼容 UDA 两种 source
        ...
        if with_smm[0]:
            with_smm, smm_stage, smm_half, smm_in, smm_lam = with_smm
            self.smm_stage = smm_stage
            if smm_stage == -1:
                dim = 3
            else:
                dim = [64, 256, 512, 1024, 2048][smm_stage]
            self.smm = SMMLayer(dim, self.n_source, smm_half, smm_in, smm_lam)
        else:
            self.smm = nn.Identity()
            self.smm_stage = -2
        ...
        if not self.cut_at_pooling:
            ...
            self.num_classes = num_classes[0] if isinstance(num_classes, list) else num_classes
...
def resnet_plus(depth, pretrained=True, cut_at_pooling=False,
    ...
    with_smm      = [args.with_smm, args.smm_stage, args.smm_half, args.smm_in, args.smm_lam]
    depth         = depth
    # fmt: on
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_se, with_nl, with_in,
                   ...
                   cut_at_pooling, num_features, norm, dropout, num_classes)
    if pretrain:
...
def resnet50_plus(**kwargs):
    return resnet_plus('50x', **kwargs)
