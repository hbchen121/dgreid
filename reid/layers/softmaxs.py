...
def cosSoftmax(logits, targets, m):
...
def arcSoftmax(logits, targets, m):
    index = torch.where(targets != -1)[0]
    m_hot = torch.zeros(index.size()[0], logits.size()[1], device=logits.device, dtype=logits.dtype)
    m_hot.scatter_(1, targets[index, None], m)
    logits.acos_()
...
def circleSoftmax(logits, targets, m):
...
class CosSoftmax(Linear):
    ...
    def forward(self, features, targets=None):
        logits = F.linear(F.normalize(features), F.normalize(self.weight))
        logits = cosSoftmax(logits, targets, self.m)
        logits.mul_(self.s)
        return logits
...
class ArcSoftmax(Linear):
    ...
    def forward(self, features, targets=None):
        logits = F.linear(F.normalize(features), F.normalize(self.weight))
        logits = arcSoftmax(logits, targets, self.m)
        logits.mul_(self.s)
        return logits
...
class CircleSoftmax(Linear):
    ...
    def forward(self, features, targets=None):
        logits = F.linear(F.normalize(features), F.normalize(self.weight))
        logits = circleSoftmax(logits, targets, self.m)
        logits.mul_(self.s)
        return logits
