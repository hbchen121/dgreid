...
from reid.utils.data.comm_dataset import CommDataset, SplitDataset
...
from reid.utils.relabel import ReLabel_identity as ReLabel
...
def main_worker(args):
    ...
    source_nclass = []
    subset_num = 3
    dataset = get_data(args.dataset_source, args.data_dir, args.combine_all)
    dataset_source = SplitDataset(dataset.train, verbose=True)
    ...
    for train_set in dataset_source.train:
    ...
    source_nclass = [0 for _ in range(subset_num)]
    source_nclass[0] = dataset.num_train_pids
    args.nsource = subset_num
    # args.nclass = [0 for _ in range(subset_num)]
    # args.nclass[0] = dataset.num_train_pids
