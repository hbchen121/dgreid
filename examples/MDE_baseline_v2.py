...
from reid.trainers.mde_trainers_v2 import MDE_Trainer
...
def main_worker(args):
    ...
    #             # print(name)
    #             params.append({"params": [value], "lr": args.lr * 2})
    #         else:
    #             params.append({"params": [value]})
    params = [{"params": [value]} for _, value in model.named_parameters() if value.requires_grad]
    # cls_params = [{"params": [value], "lr": args.lr * 2} for _, value in classifier.named_parameters() if value.requires_grad]
    ...
    # lr_scheduler = WarmupMultiStepLR(optimizer, milestones=[40, 80], gamma=0.1, warmup_factor=0.01,
    #                                  warmup_iters=10, warmup_method="linear")
    ...
    for epoch in range(args.epochs):
        ...
        if not args.combine_all or args.local_lambda > 0:
